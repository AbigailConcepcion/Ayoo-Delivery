    ...
    <div className="flex flex-col h-screen bg-gray-50 overflow-hidden">
      <div className="bg-white p-6 flex items-center justify-between shadow-sm z-10 border-b border-gray-100">
         <div className="flex items-center gap-4">
            <button onClick={onBack} className="w-10 h-10 bg-purple-50 rounded-xl flex items-center justify-center text-[#6D28D9] text-2xl font-black">←</button>
            <h2 className="text-xl font-black uppercase tracking-tighter">My Locations</h2>
         </div>
         {isLoading && (
            <div className="w-5 h-5 border-3 border-[#6D28D9] border-t-transparent rounded-full animate-spin"></div>
         )}
      </div>
      ...
      <div className="flex-1 p-6 overflow-y-auto scrollbar-hide space-y-6">
         {isLoading && addresses.length === 0 ? (
           ...
           addresses.map(a => (
             <div key={a.id} className="bg-white p-7 rounded-[40px] shadow-sm flex items-center justify-between border border-gray-50 group animate-in slide-in-from-right-5 hover:border-[#6D28D9]/30 transition-all">
                <div className="flex-1">
                 <div className="flex items-center gap-2 mb-2">
                     <h4 className="font-black text-gray-900 tracking-tight leading-none text-lg uppercase">{a.label}</h4>
                     <span className="bg-purple-50 text-[#6D28D9] px-2 py-0.5 rounded-lg text-[8px] font-black uppercase">{a.city}</span>
                ...
                <div className="flex gap-2">
                  ...
                  <button 
                    onClick={() => startEdit(a)}
                    className="w-10 h-10 bg-purple-50 text-[#6D28D9] rounded-xl flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all hover:bg-purple-100 active:scale-90"
                  >
                    ✎
                  </button>
                  <button 
         ...
         {showForm && (
           <div className="bg-white p-8 rounded-[45px] shadow-2xl border-2 border-[#6D28D9]/10 animate-in zoom-in-95">
              ...
              <div className="space-y-6">
                 <div className="input-label-border">
                    ...
                    <input 
                      ...
                      className="w-full p-5 bg-gray-50 border border-gray-100 rounded-[24px] focus:border-[#6D28D9] outline-none font-black text-sm uppercase transition-all"
                    />
                 </div>
                 <div className="input-label-border">
                    ...
                    <select 
                      ...
                      className="w-full p-5 bg-gray-50 border border-gray-100 rounded-[24px] focus:border-[#6D28D9] outline-none font-black text-sm uppercase appearance-none"
                    >
                 ...
                 <div className="input-label-border">
                    ...
                    <input 
                      ...
                      className="w-full p-5 bg-gray-50 border border-gray-100 rounded-[24px] focus:border-[#6D28D9] outline-none font-black text-sm transition-all"
                    />
                 </div>
                 {MAP_API_KEY ? (
                   <div className="space-y-3">
                     ...
                     {distanceKm !== null && estFee !== null && (
                       <div className="bg-purple-50 p-4 rounded-2xl border border-purple-100">
                         <p className="text-[10px] font-black uppercase tracking-widest text-purple-500">Auto Delivery Estimate</p>
