...
const History: React.FC<HistoryProps> = ({ onBack, orders, onNavigate }) => {
   ...
   useEffect(() => {
      if (!selectedActiveOrder || selectedActiveOrder.status !== 'OUT_FOR_DELIVERY') {
         return;
      }
      ...
      const interval = setInterval(() => {
         setRiderProgress(prev => {
            ...
            setRiderLocation({
               lat: restaurantLocation.lat + (deliveryLocation.lat - restaurantLocation.lat) * next,
               lng: restaurantLocation.lng + (deliveryLocation.lng - restaurantLocation.lng) * next
            });
            return next;
         });
      }, 3000);
   ...
   }, [selectedActiveOrder?.id, selectedActiveOrder?.status, restaurantLocation, deliveryLocation]);
            ...
            <div className="flex gap-2 mt-6">
               <button
                  ...
                  className={`flex-1 py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${activeTab === 'LIVE'
                        ? 'bg-white text-[#C084FC] shadow-lg'
                     ...
                     {activeOrders.length === 0 ? (
                        ...
                        activeOrders.map((order) => (
                           <div
                              ...
                              className="bg-white border-2 border-gray-100 rounded-[30px] p-5 shadow-sm hover:shadow-lg hover:border-[#C084FC]/30 transition-all cursor-pointer"
                           >
                              ...
                              <div className="flex items-center gap-3 mb-3">
                                 <div className="w-10 h-10 bg-gradient-to-br from-[#C084FC] to-[#A855F7] rounded-2xl flex items-center justify-center text-white text-lg">
                                    🍔
                                 </div>
                                 <div>
